...
var TransmissionClient = include('transmission'),
    ...
    TorrentProjectClient = include('torrentProvider/torrentProject'),
    TorrentProjectTorrentProvider = include('torrentProvider/torrentProjectProvider');

export function resolve (taskName, settings, fileManager, logger) {
    if (taskName === 'tv') {
...
function resolveSubsDownload (settings, fileManager, logger) {
    var TvLibrary = include('library'),
        Notifications = include('notifications'),
        SubtitlesDownloader = include('subs/downloader');

    var library = new TvLibrary(settings, fileManager);
    var downloader = new SubtitlesDownloader(settings, fileManager, library, new Notifications(settings), logger);
...
function resolveTvShowsDownload (settings, fileManager, logger) {
    ...
    .then(function () {
        ...
        var torrentClient = new TorrentProjectClient();
        var torrentSearch = new TorrentProjectTorrentProvider(torrentClient);
